...
echo ""
echo "Agent Forwarding (bastion -> webserver):"
echo "  ssh-add ssh_keys/ansible_key                           # Register key to ssh-agent"
echo "  ssh -A -i ssh_keys/ansible_key -p 2222 ansible@localhost"
echo "  (on bastion) ssh ansible@172.20.0.20"
echo ""
echo "User: ansible (SSH key authentication only)"
